...
public function get_elementos($estado_reporte,$catego_1,$autor_1,$nick_1,$fecha_ini,$fecha_fin,$array_results){
...
// fecha y hora del sistema
    ...
    foreach ($array_results as $fila){
        ...
        $yTupla=$this->GetY();
        if($yTupla+$Nfila>18){  $this->SetAutoPageBreak(true,2);}
        
        if($yTupla>18){$this->SetAutoPageBreak(true,2);}
        $this->SetXY($x1,$yTupla+$Nfila);
        $this->Cell(1,1,utf8_decode($fila['ID']),'1','0','C','0');
        $yTupla=$this->GetY(); 
        $Nfila = 0;
        $this->MultiCell($TCell+$W,1,utf8_decode($fila['post_title']),'1','0','C','0');
        $AltoTitulo=$this->GetY(); // calculo de alto de celda del titulo
        $AltoTitulo = $AltoTitulo - $yTupla;
        
        if($AltoTitulo>1){$Nfila = $Nfila + $AltoTitulo - 1;}else{$Nfila =0;}
        ...
        $this->Cell(2,1,utf8_decode($fila['post_status']),'1','0','C','0');
        }
        //$this->SetXY('10',$y);
        if($rol_print!='si'){ //$this->SetXY('10',$y);
        $this->Cell(3.25,1,utf8_decode($fila['Rol_Autor']),'1','0','C','0');
        }
        if($nick_print!='si'){ $this->Cell(2.75,1,utf8_decode($fila['alias']),'1','0','C','0');}
        ...
        $this->Cell(4,1,$AltoTitulo.' '.$yTupla.' '.$Nfila,'1','0','C','0');
        if($yTupla>18){$this->SetAutoPageBreak(true,2);}
        $this->Ln(1);
        $contador = $contador + 1;
